...
# Model D #
###########
checkPoint <- callback_model_checkpoint(filepath = file.path(work_folder, "lob_D.h5"),
                                        monitor = "val_acc", save_best_only = TRUE)

reduceLr <- callback_reduce_lr_on_plateau(monitor = "val_acc", factor = 0.1, patience = 3)
logger <- callback_csv_logger(file.path(work_folder, "logger-lob_D.csv"))
...
# use w = 100 most recent states of limit order book and 20 volume data (10 levels for each bid/ask: 2*10 = 20)
k_clear_session()

row_hidden <- 32
col_hidden <- 32

input <- layer_input(shape = c(w, 20, 1))

# Encodes a row of pixels using TimeDistributed Wrapper
# the axis right after the sample axis will be considered to be the temporal axis
encoded_rows <- input %>% time_distributed(layer_cudnn_lstm(units = row_hidden))

# Encodes columns of encoded rows
encoded_columns <- encoded_rows %>% layer_cudnn_lstm(units = col_hidden)
...
his <- model %>% fit_generator(sampling_generator(X_data_train, Y_data_train, batch_size = batch_size, w=w),
                               ...
                               callbacks = list(checkPoint, reduceLr, logger, earlyStop),
                               validation_data = sampling_generator(X_data_val, Y_data_val, 
                                                                     batch_size = batch_size, w=w),
                               validation_steps = 100)
plot(his)


fitted_D <- load_model_hdf5(file.path(work_folder, "lob_D.h5"))

results_D <- fitted_D %>% evaluate_generator(sampling_generator(X_data_test, Y_data_test, batch_size = batch_size, w=w), 
                                             steps = 1000)
results_D

fmlr::acc_lucky( table(Y_data_train)[c(2,3)], table(Y_data_test)[c(2,3)], results_D$acc)
