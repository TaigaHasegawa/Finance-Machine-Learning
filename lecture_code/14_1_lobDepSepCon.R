...
# Model E #
###########
checkPoint <- callback_model_checkpoint(filepath = file.path(work_folder, "lob_E.h5"),
                                        monitor = "val_acc", save_best_only = TRUE)

reduceLr <- callback_reduce_lr_on_plateau(monitor = "val_acc", factor = 0.1, patience = 3)
logger <- callback_csv_logger(file.path(work_folder, "logger-lob_E.csv"))
...
summary(model)  # it has a relatively less number of parameters 
...
fitted_E <- load_model_hdf5(file.path(work_folder, "lob_E.h5"))

results_E <- fitted_E %>% evaluate_generator(sampling_generator(X_data_test, Y_data_test, batch_size = batch_size, w=w), 
                                             steps = 1000)
results_E

fmlr::acc_lucky( table(Y_data_train)[c(2,3)], table(Y_data_test)[c(2,3)], results_E$acc)
